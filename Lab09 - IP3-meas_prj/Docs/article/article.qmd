...
$$
\begin{split}
  \text{IIP}_{3h}&=2\sqrt{\left|\frac{I_s/V_T}{I_s/\left(6V_T^3\right)}\right|}\implies\\
  &=2V_T\sqrt{6}
\end{split}
$$ {#eq-iip3h-diode}
...
$$
\begin{split}
  \text{IIP}_{3i}&=\frac{2V_T\sqrt{6}}{\sqrt{3}}\implies\\
  &=2V_T\sqrt{2}\implies\\
\end{split}
$$ {#eq-iip3i-diode}
...
a1_bjt_ideal_0 = Is/VT
a2_bjt_ideal_0 = Is/VT**2/2
a3_bjt_ideal_0 = Is/VT**3/6

g_bjt_ideal = gm

IIP3h_bjt_ideal = 2*math.sqrt(abs(a1_bjt_ideal_0)/abs(a3_bjt_ideal_0))
...
De acordo com o problema proposto, deseja-se quadruplicar o $\text{IIP}_{3i}$ do circuito com o BJT ideal. Dessa forma, o valor desejado é $4\text{IIP}_{3i}=$ `{python} format_simple(4*IIP3i_bjt_ideal, 4)` V. Para isso, será utilizado o conceito de degeneração de emissor. A degeneração de emissor é causada pela inclusão de resistência em série com o terminal do emissor do transistor. Essa degeneração gera *feedback* negativo no amplificador e reduz o $IIP_3$ do circuito (sob a pena de reduzir um pouco o ganho do amplificador). O circuito com o resistor de emissor, $R_E$, incluso, pode ser visualizado na [Fig. @].
...
\begin{figure}[h]
  ...
  \begin{circuitikz}[american]
    ...
    \draw (-0.5, -4.5) node[ground]{} to[sV, l=$V_{s2}$] ++(0, 1.5) to[sV, l=$V_{s1}$] ++(0, 2.5) to[battery1, l=$V_B$, invert] (nmos1.B) ;
...
para $a_2^{'}$, com $a_2=g_m/V_T$

$$
\begin{split}
  a_2^{'}&=\frac{g_m/V_T}{\left(1+R_Eg_m\right)^3}
\end{split}
$$

e, para $a_3^{'}$, com $a_3=g_m/V_T^2$

$$
\begin{split}
  a_3^{'}&=\frac{g_m/V_T^2}{\left(1+R_Eg_m\right)^4}-\frac{2\left(g_m/V_T\right)^2R_E}{\left(1+R_Eg_m\right)^5}
\end{split}
$$
