...
\begin{split}
  ...
  \text{IP}_{3i}&=V_i=2\sqrt{\frac{\left|a_1\right|}{3\left|a_3\right|}}\\
...
a2_diode = Is/VT**2/2 
a3_diode = Is/VT**3/6
IP3h_diode = 2*math.sqrt(abs(a1_diode)/abs(a3_diode))
IP3i_diode = IP3h_diode/math.sqrt(3)
```

Dadas a [Eq. @eq-def-a1] e a [Eq. @eq-def-a3], pode-se quantificar os pontos de interceptação de terceira ordem (harmônico e intermodulação). Dessa forma, $\text{IP}_{3h}=$ `{python} format_simple(IP3h_diode, 4)` V e $\text{IP}_{3i}=$ `{python} format_simple(IP3i_diode, 4)` V.
...
# Intervalo de Vi
Vi = np.logspace(np.log10(700e-6), np.log10(2), 1000)

a1_diode_generic = yD
a3_diode_generic = yD/(6*VT**2)

# Cálculo de Io
Io1 = a1_diode_generic * Vi
Io3 = 3/4 * a3_diode_generic * Vi**3
...
plt.axhline(I_IP3, color="red", linestyle=":", label=f"$OIP3 = {I_IP3:.2e}$ A")
plt.axvline(Vi_IP3, color="purple", linestyle=":", label=f"$IIP3 = {Vi_IP3:.2e}$ V")
