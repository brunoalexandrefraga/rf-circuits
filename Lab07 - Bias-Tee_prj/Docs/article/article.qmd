...
Logo de início, já é possível verificar que a tensão $V_1$ é a tensão que opera sobre a resistência de carga, ou seja, $V_1=V_{L1}$. Sabendo $V_1$, também é possível obter a corrente que opera sob essa resistência, cuja relação é dada a seguir

$$
 I_{L1}=\frac{V_{L1}}{R_L}=\frac{V_1}{R_L}
$$
...
$$
  \begin{split}
    ...
    \frac{V_s-V_1}{R_s+X_c}&=\frac{V_1}{X_L}+\frac{V_1}{R_L}\implies\\
    \frac{V_s}{R_s+X_c}&=V_1\left(\frac{1}{X_L}+\frac{1}{R_L}+\frac{1}{R_s+X_c}\right)\implies\\
    \frac{V_s}{R_s+X_c}&=V_1\left[\frac{(R_L+X_L)(R_s+X_C)+X_LR_L}{X_LR_L(R_s+X_C)}\right]\implies\\
    V_s&=V_1\left[\frac{(R_L+X_L)(R_s+X_C)+X_LR_L}{X_LR_L}\right]\implies\\
    V_1&=\frac{V_sX_LR_L}{(R_L+X_L)(R_s+X_C)+X_LR_L}\implies\\
  \end{split}
$$

em que $I_L=I_{L1}$, a impedância do indutor é $X_L=j\omega L$ e a impedância do capacitor é $X_C=\left(j\omega C\right)^{-1}$.