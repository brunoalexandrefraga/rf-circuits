...
![Insertion loss com Monte Carlo](../images/monte-carlo-IL1dB.png){width=300, #fig-monte-carlo-IL1dB}
...
Como foi obtido um valor de 2,02 dB na simulação apresentada na [Fig. @fig-monte-carlo-IL1dB], um valor que pode ser adequado seja de 0,5 dB. Dessa forma, o projeto deve ser refeito, considerando que a perda por inserção não deva ultrapassar 0,5 dB na frequência de 100 kHz.

Para que o circuito seja devidamente obtido, primeiramente calcula-se o capacitor por meio da equação [Eq. @eq-capacitor-value]. Dessa forma, obtém-se um capacitor de $C =$ `{python} format_capacitance(calc_capacitance(f=100e3, IL_ideal=0.3))`. Semelhantemente, utiliza-se a [Eq. @eq-insertion-loss] para o cálculo do indutor, levando a um indutor $L =$ `{python} format_inductance(calcular_indutancia(f=100e3, C=4.5562481787255145e-08, IL_dB=0.3, R_L=50, R_s=50))`.

$C =$ `{python} calc_capacitance(f=100e3, IL_ideal=0.3)`.
$L =$ `{python} calcular_indutancia(f=100e3, C=5.9512540245923864e-08, IL_dB=0.3, R_L=50, R_s=50)`.