...
$$
  \begin{split}
    \sqrt{\text{IL}}&=\frac{R_sX_L^{'}-R_LX_C^{'}+X_L^{'}R_L-j(R_LR_s+X_L^{'}X_C^{'})}{X_L^{'}R_s+X_L^{'}R_L}\implies\\
    \text{IL}&=\frac{(R_sX_L^{'}-R_LX_C^{'}+X_L^{'}R_L)^2+(R_LR_s+X_L^{'}X_C^{'})^2}{\left(X_L^{'}\right)^2(R_s+R_L)^2}
  \end{split}
...
Outro ponto a se verificar é em relação à simulação.