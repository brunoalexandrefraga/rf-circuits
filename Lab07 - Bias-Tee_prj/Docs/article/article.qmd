...
Supondo que não seja possível selecionar um componente com uma menor tolerância, uma forma de solucionar essa extrapolação é projetando o circuito para que esse a perda por inserção, sem considerar a tolerância, seja de menos de 1 dB. Como foi obtido um valor de 2,02 dB na simulação apresentada na [Fig. @fig-monte-carlo-IL1dB], um valor que pode ser adequado seja de 0,5 dB. Dessa forma, o projeto deve ser refeito, considerando que a perda por inserção não deva ultrapassar 0,5 dB na frequência de 100 kHz.
...
L=`{python} calcular_indutancia(f=100e3, C=4.5562481787255145e-08, IL_dB=0.5, R_L=50, R_s=50)`