...
Com a corrente $I_E$ obtida anteriormente, obtém-se uma tensão $V_{BE}=$ `{python} format(VBE)`V. Assumindo-se uma tensão $V_{{CE}_\text{sat}}=$ `{python} format_simple(VCEsat, 1)`V, obtém-se uma tensão $V_{CC}=$ `{python} format(VCC)`V. Para garantir a correta operação do amplificador, é necessário uma corrente de base maior que $V_{BE}$, dessa forma, seleciona-se uma tensão de base, $V_B=$ `{python} format(VB)`V.
