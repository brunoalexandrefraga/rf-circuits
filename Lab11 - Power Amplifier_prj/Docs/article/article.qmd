...
Para se obter a tensão de base desejada e selecionando um resistor $R_{B2}=$ `{python} format(RB2)`$\Omega$, chega-se a $R_{B1}=$ `{python} format(RB1)`$\Omega$. Para calcular o resistor de emissor, utiliza-se a seguinte equação
...
Dessa forma, obtém-se um resistor $R_E=$ `{python} format(RE)`$\Omega$.
