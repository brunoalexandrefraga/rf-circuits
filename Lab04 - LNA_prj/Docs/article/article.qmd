...
Dessa forma, acopla-se esse amplificador emissor-comum ao *testbench* apresentado. Variando-se a corrente de emissor $I_E$ de 10 $\mu\text{A}$ a 200 $\mu\text{A}$, obtém-se a curva apresentada na [Fig. @fig-testbench-gain-result], além do valor de corrente de `{python} format(ie_13dB)`A.

![Resultados do ganho](../images/testbench-gain-result.png){width=300, #fig-testbench-gain-result}

A seguir, é apresentado o raciocínio utilizado para polarizar o amplificador com a corrente determinada anteriormente. A Fig.\ref{fig:generig-ampl} apresenta o circuito padrão para o projeto de um amplificador emissor comum.
...
Dessa forma, $R_{B1}=$ `{python} format(RB1)`$\Omega$ e $R_{B2}=$ `{python} format(RB2)`$\Omega$. E finalmente, é possível calcular a resistência do coletor,
...
resultando em $R_C=$ `{python} format(RC)`$\Omega$.
