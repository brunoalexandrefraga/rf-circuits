...
A rede de adaptação acoplada a um circuito de verificação pode ser visualizada na [Fig. @fig-network-adapter-verification]. Pode parecer que a rede de adaptação está invertida em relação à Fig.\ref{fig:matching-network}, mas é a mesma, uma vez que é necessário que o indutor esteja em paralelo com a carga.

![Circuito de validação da rede de adaptação projetada](../images/network-adapter-verification.png){width=300, #fig-network-adapter-verification}

A aplicação dos valores obtidos para o capacitor e para o indutor, além do resultado obtido com o acoplamento da rede de ataptação podem ser visualizados na [Fig. @fig-network-adapter-verification-result].
...
Os resultados apresentados confirmam todos os cálculos que foram feitos até então.

Aplicando os valores à equação, obtém-se $C_1 =$ `{python} format(C1)`F. Em seguida, na @fig-complete-network-adapter-shuntC, é apresentado os resultados com esse capacitor acoplado ao circuito de teste.
